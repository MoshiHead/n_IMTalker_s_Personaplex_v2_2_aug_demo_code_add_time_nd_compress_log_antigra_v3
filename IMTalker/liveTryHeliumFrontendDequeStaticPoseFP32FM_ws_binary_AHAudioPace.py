...
class LiveHeliumFMEngine:
    ...
    def render_and_encode_subbatch(
        ...
        for j, fut in enumerate(jpeg_futures):
            ...
            flags = 1 if is_mouth_moving else 0
                        ...
                        with contextlib.suppress(Exception):
                            ...
                            if rec is not None:
                                ...
                                if "avatar_first_mouth_movement_s" in payload:
                                    ...
                                    real_avatar_lat = t_send_offset + f_mouth_s
                                    # Print real visible response summary
                                    print(
                                        f"\n[AVATAR LATENCY] TURN {turn_id}:\n"
                                        f"  ACTUAL FIRST RESPONSE LATENCY: {real_avatar_lat:.3f}s\n"
                                        f"  (USER_SPEECH_END -> AVATAR_FIRST_MOUTH_MOVEMENT)\n",
                                        flush=True,
                                    )
                    continue
