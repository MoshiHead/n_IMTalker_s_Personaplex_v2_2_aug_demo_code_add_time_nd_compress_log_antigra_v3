...
class MoshiOnlyEngineWithHidden(MoshiOnlyEngine):
    ...
    def _install_graph_hidden_capture(self) -> None:
        ...
        if hasattr(lm_gen, "prepare_step_input") and hasattr(lm_gen, "process_transformer_output"):
            ...
            def personaplex_step_with_hidden(
                ...
                # Feeds the context-refresh replay buffer (see lm.py:
                # refresh_context_async). This path bypasses the public
                # step()/_step_impl() wrapper that normally does this
                # recording, so it must be done here explicitly.
                if self_gen.record_history and output is not None:
                    self_gen._history.append(output[0, :, 0])
        ...
        # This fallback also bypasses the RingKVCache-shaped streaming state
        # that save_prompt_snapshot/refresh_context_async operate on (it uses
        # its own state.offsets/state.cache.gather bookkeeping), so context
        # refresh cannot run here either. _arm_context_refresh() only checks
        # hasattr(lm_gen, "save_prompt_snapshot") -- which is still True on
        # this path -- so disable it explicitly to avoid corrupting this
        # branch's state.
    ...
    def _route_and_search(self, transcript: str, my_epoch: int, rules_said_search: bool) -> None:
        ...
        try:
            ...
            lookup_text = search_helpers.wrap_with_lookup_tags(my_epoch)
    ...
    def _open_turn_latency(self, turn_id: int, transcript: str) -> None:
        ...
        try:
            ...
            # Which context_refresh generation was active when this turn's
            # generation ran -- the field that lets a bad answer in the log
            # be correlated with "was a context_refresh (especially a
            # zero-history one) recently active?" instead of having to guess
            # from timestamps (see conversation_log_1's investigation, where
            # every one of turns 15/22's corrupted answers landed in the
            # window right after a context_refresh line).
            self.conv_logger.latency.count(
                turn_id, context_version=int(getattr(self, "context_version", 0)))
    ...
    def _step(self, pcm24: np.ndarray) -> dict:
        ...
        # Long-prompt / long-conversation stability (see liveTry.py:
        # _maybe_refresh_context). Uses model_own_rms, not reply_rms, so a
        # thinking-sound filler swapped in during a search never masquerades
        # as "the model is speaking" and blocks the quiet-moment trigger.
        self._maybe_refresh_context(in_rms=input_rms, out_rms=model_own_rms, text_token=token)
    ...
    def initialize(self, parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        ...
        parser.add_argument(
            ...
            default=os.environ.get(
                "PERSONAPLEX_GREETING_TEXT", "Hello. How can I help you today?"
            ),
            help=(
                "Deterministic opening line spoken once per new session, instead of "
                "letting the model free-run into an unconditioned continuation of the "
                "system prompt (see liveTry.py:_emit_greeting). Set to the exact "
                "persona greeting this deployment wants, or '' to disable and restore "
                "the old free-running behavior."
            ),
        )
    ...
    def get_moshi_engine() -> MoshiOnlyEngine:
        ...
        if moshi_engine is None:
            moshi_engine = MoshiOnlyEngineWithHidden(
                ...
                greeting_text=getattr(args, "greeting_text", ""),
