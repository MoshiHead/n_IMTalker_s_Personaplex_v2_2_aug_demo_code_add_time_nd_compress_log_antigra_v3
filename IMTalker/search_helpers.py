...
# A handful of paraphrases rather than one fixed sentence. Root cause found in
# conversation_log_1: the exact string "<lookup> Please wait a minute." was
# force-fed into the live context identically on every single search turn (6
# times within about 4 minutes). By turn 17 the model was reproducing "please
# wait a minute" VERBATIM as its own genuinely-generated speech ("...tricky
# one to understand. Please wait a minute while I think about it."), and the
# very next turns (18/19/21) then enacted the waiting it had just described as
# real silence -- a 11-31s freeze with zero spoken words each time (see
# lm.py's text_silence_break_steps for the generation-side fix; this reduces
# how strongly the model learns "please wait" as its own catchphrase in the
# first place by not repeating one identical sentence turn after turn).
_LOOKUP_PHRASES = (
    "<lookup> One moment.",
    "<lookup> Let me check.",
    "<lookup> Give me a second.",
    "<lookup> Checking that now.",
)


def wrap_with_lookup_tags(turn_index: int = 0) -> str:
    return _LOOKUP_PHRASES[int(turn_index) % len(_LOOKUP_PHRASES)]
...
# and repeatedly since (conversation_log_1: ",.>", "an.>", "not what I.>" as
# the leading fragment of turns 15/12/6 -- a trailing '.>'/'>' anywhere in the
# text, not only at the very start, since the turn boundary can land mid-tag
# after real words the model also said in the same breath). Stripping them
# keeps the recorded reply readable and stops tag text being mistaken for
# something the user heard.
#
# NOTE this is presentation only, for the LOGGED transcript. It cannot stop
# the model from *speaking* an echoed tag out loud; that is addressed at the
# generation side instead (decode_piece's BOS/EOS/byte-fallback filtering in
# liveTry.py, and varying wrap_with_lookup_tags() so the model has less
# identical injected markup to imitate in the first place).
_ECHOED_TAG_RE = re.compile(
    r"<\s*/?\s*(?:ref|lookup|system|s|unk|pad|bos|eos|0x[0-9A-Fa-f]{2})\s*>", re.IGNORECASE
)
# A bare '>' is not valid spoken-English punctuation -- it is reliably the
# tail of a tag that got split across a turn boundary, optionally preceded by
# trailing punctuation/whitespace from the word it landed against ("not what
# I.>", "an.>", ",.>" -- all seen verbatim in conversation_log_1). Matched
# anywhere in the string, not only at the start.
_ORPHAN_TAG_TAIL_RE = re.compile(r"[\s.,]*>+")


def strip_injected_tags(text: str) -> str:
    """Remove echoed <ref>/<lookup>/<system>/BOS/EOS markup and any stray
    '.>'/'>' fragment left when a turn boundary lands mid-tag, wherever in
    the text it appears."""
    if not text:
        return ""
...
# Minimum fraction of the compressor's own answer (by content word, i.e.
# excluding stopwords) that must actually appear somewhere in the passages it
# was given. See compressor_faithfulness_overlap's docstring and the
# ContextCompressor.compress() call site for why this exists and why it is a
# content-word overlap rather than a raw-word one.
COMPRESSOR_OVERLAP_MIN = 0.25


def compressor_faithfulness_overlap(answer: str, chunks: list[dict]) -> float:
    """Fraction of `answer`'s content words that appear somewhere in `chunks`'
    text. A pure function (no model, no I/O) specifically so this faithfulness
    gate is unit-testable without loading the compressor's LLM -- see
    test_search_helpers.py's test replaying conversation_log_1 turn 6
    verbatim (the "Aravilev" invented-claim example this gate exists for)."""
    passage_words = {w for c in chunks for w in _content_words(c.get("text", ""))}
    answer_words = _content_words(answer)
    return len(answer_words & passage_words) / max(1, len(answer_words))
...
class ContextCompressor:
    ...
    def compress(self, question: str, chunks: list[dict]) -> str:
        ...
        user_content = (
            ...
            "- Only state facts that are directly supported by the passages. Never guess, "
            "infer, or make a claim about any person, company, or product that is not "
            "actually described in the passages -- even if that name appears in the "
            "question itself. If the question names something the passages say nothing "
            "about, that is exactly the NO_CONTEXT case below, not something to answer "
            "from assumption.\n"
            ...
            "- If the passages genuinely do not answer the question -- including when they "
            "simply don't mention the specific person/company/product being asked about -- "
            "reply exactly NO_CONTEXT. Do not soften this into a sentence like \"X does not "
            "provide Y\"; that is still an invented claim about X.\n"
            f"Q: {question}\nPassages:\n{passages}\nA:"
        )
        ...
        # Content-word overlap, not raw word overlap. The previous version
        # counted every word including stopwords ("no", "does", "not", "in",
        # "on", ...), which dominate a short sentence and inflate the score
        # for an answer that has almost nothing to do with the passages --
        # confirmed in conversation_log_1 turn 6: for the question "So,
        # Aravilev, can you help me to invest in Bitcoin?" against two
        # ordinary "how to invest in Bitcoin" passages that never mention
        # "Aravilev" at all, the compressor invented "No, Aravilev does not
        # provide information on investing in Bitcoin." -- and raw-word
        # overlap (dominated by "bitcoin", "in", "on", "not", "does") passed
        # it straight through the 0.15 threshold. Content-word overlap for
        # that exact answer is ~0.17 (1 of 6 content words -- "bitcoin" --
        # actually appears in the passages; "aravilev" does not), so raising
        # the bar to 0.25 on the filtered signal rejects it while still
        # passing every real grounded answer logged in that same session
        # (Bitcoin/BULL/Google/EUR price answers all score >=0.3 content-word
        # overlap against their own passages).
        overlap = compressor_faithfulness_overlap(result, chunks[: self.max_passages])
        self.last_stats["compressor_passage_overlap"] = round(overlap, 3)
        if overlap < COMPRESSOR_OVERLAP_MIN:
            print(
