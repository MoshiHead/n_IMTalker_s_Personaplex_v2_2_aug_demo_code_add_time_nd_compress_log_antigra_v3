...
# Special/control token ids: 0=EPAD, 1=BOS, 2=EOS, 3=PAD (see moshi/models/lm.py,
# LMGen.zero_text_code=3 and the "0..3 = EPAD/BOS/EOS/PAD" comment in
# process_transformer_output). The live pipeline used to only special-case 0
# and 3 -- if the model ever SAMPLED id 1 or 2 (observed in production: a
# degenerate/just-refreshed context makes this far more likely), decode_piece
# happily turned it into the literal SentencePiece string "<s>" or "</s>" and
# that string was then spoken and logged as if it were real content
# (conversation_log_1 turn 0: "<s><s> Oh, it's so funny..."; turn 22: a
# hidden-prompt-shaped leak immediately preceded by "<0x0A>"). Byte-fallback
# SPM pieces like "<0x0A>" are a second, separate leak: they are ordinary
# vocabulary ids (not 0-3) whose *decoded piece* happens to look like markup.
# Both need filtering at the token/decode boundary, not by regexing the
# final transcript -- by the time text reaches the transcript it has already
# been "spoken" (become part of self.audio_text / the model's own visible
# history), so this is the actual place raw control output can be stopped
# rather than just hidden.
_SPECIAL_TEXT_TOKEN_IDS = frozenset((0, 1, 2, 3))
_CONTROL_PIECE_RE = re.compile(
    r"^\s*<\s*/?\s*(?:s|unk|pad|0x[0-9A-Fa-f]{2}|ref|lookup|system)\s*>\s*$",
    re.IGNORECASE,
)


def _is_control_piece(piece: str) -> bool:
    """True for a decoded SPM piece that is markup/control output rather than
    spoken content: BOS/EOS/UNK/PAD spellings, raw byte-fallback tokens
    ("<0x0A>"), or an echoed injection tag sampled as a single piece."""
    return bool(_CONTROL_PIECE_RE.match(piece or ""))
...
class MoshiOnlyEngine:
    def __init__(
        ...
        *,
        ...
        from moshi.utils.diagnostics import (
            _IN_SPEECH_RMS, _OUT_SPEECH_RMS, _env_flag, _env_float, _env_int,
        )
        self._refresh_in_speech_rms = _IN_SPEECH_RMS
        self._refresh_out_speech_rms = _OUT_SPEECH_RMS

        # --- Long-prompt / long-conversation stability -----------------------
        # The model's RoPE positions are absolute and it collapses into
        # permanent silence once its streaming offset passes the maximum
        # position seen in training (~6100 steps, ~8 minutes at 12.5
        # steps/s -- much sooner once a long system prompt has already burned
        # hundreds/thousands of those steps before the user ever spoke, which
        # is why a long prompt used to break the session almost immediately).
        # `_maybe_refresh_context` (called from `_step`) periodically restores
        # the post-prompt KV/offset snapshot taken in `_apply_system_prompts`
        # and replays recent dialogue, batched, well before that cliff. See
        # moshi/models/lm.py: save_prompt_snapshot / refresh_context_async.
        ...
        # Silence breaker: forces a real word once a turn goes this many
        # consecutive SAMPLED (non-forced) EPAD/PAD steps -- ~14s at the
        # default of 175 steps (12.5 steps/s). Root-caused in
        # conversation_log_1: turns decided instantly with nothing to inject
        # ("ANSWER DIRECTLY via=rules" -> immediate "DONE") produced zero
        # spoken words for 11-31s, repeatedly, because the model had started
        # imitating its own recently-injected "<lookup> Please wait a
        # minute." filler as if it were something IT had just said, and then
        # enacted the implied waiting as real silence (see turn 17's actual
        # spoken reply: "...Please wait a minute while I think about it.",
        # immediately followed by the turn-18/19/21 freeze cluster). This is
        # a faster, per-turn fix; PERSONAPLEX_RECOVERY_SILENCE_SECS below
        # stays as the session-level last resort if it somehow doesn't help.
        self.text_silence_break_steps = _env_int("PERSONAPLEX_SILENCE_BREAK_STEPS", 175)
        # BOS(1)/EOS(2) sampled mid-stream is never legitimate and is the
        # confirmed source of the literal "<s>"/"<s><s>" leaks in
        # conversation_log_1 (turn 0, turn 22). See mask_bos_eos_mid_stream's
        # docstring in lm.py's LMGen.__init__ for the full mechanism.
        self.mask_bos_eos_mid_stream = _env_flag("PERSONAPLEX_MASK_BOS_EOS", True)
        ...
        # Armed by _apply_system_prompts() once the prompt snapshot exists.
        self._refresh: dict | None = None
        self._refresh_last_word_mono = time.monotonic()
        self._refresh_last_user_voice_mono = self._refresh_last_word_mono - 1.0
        self._refresh_quiet_run = 0
        # Bumped by _arm_context_refresh() every time the streaming state is
        # (re)armed, and by _do_context_refresh() every time it actually
        # restores a snapshot mid-session. Logged with turn/refresh events so
        # a bad answer can be correlated with exactly which context state was
        # active when it was generated (conversation_log_1's "does the
        # assistant's behavior change around a context_refresh?" question).
        self.context_version = 0

        # Deterministic startup greeting (see _emit_greeting): unset by
        # default -- an unconditioned PersonaPlex continuation after the
        # system prompt is what produced conversation_log_1 turn 0's "<s><s>
        # Oh, it's so funny. That's perfect..." (a completely uncontrolled
        # opening, not the intended persona greeting). Set this to the exact
        # greeting the deployment wants spoken first; the CLI/env default
        # below is deliberately generic (this file does not own the persona's
        # actual wording -- see IMTalker/prompts/RB_Robert_System_Prompt_full.txt).
        self.greeting_text = str(greeting_text or "").strip()
        ...
        try:
            self.lm_gen = LMGen(
                ...
                text_silence_break_steps=self.text_silence_break_steps,
                mask_bos_eos_mid_stream=self.mask_bos_eos_mid_stream,
            )
        except TypeError:
            # This fork's LMGen (moshi/models/lm.py) does not accept
            # cfg_coef/condition_tensors/on_text_hook -- the branch above
            # always TypeErrors and lands here. Retry with the kwargs it DOES
            # support (including the repetition-penalty ones) before falling
            # back to the bare minimum for a plain kyutai-moshi LMGen.
            try:
                self.lm_gen = LMGen(
                    self.lm,
                    device=self.device,
    ...
    def _apply_system_prompts(self) -> None:
        ...
        self._arm_context_refresh()
        self._emit_greeting()

    @torch.no_grad()
    def _emit_greeting(self) -> None:
        """Speak a deterministic opening line instead of letting the model
        free-run into whatever it wants right after the prompt.

        Before this fix there was no explicit startup state at all: the live
        loop just started stepping silence/mic frames through the model and
        whatever it produced first WAS the greeting. Confirmed in
        conversation_log_1 turn 0: "<s><s> Oh, it's so funny. That's perfect.
        I can only say that you got it wrong..." -- an uncontrolled
        continuation (and a BOS-token leak, separately fixed by
        mask_bos_eos_mid_stream), not a greeting. Runs once per
        `_apply_system_prompts()` call, i.e. once per new /ws/conversation
        session, which is the right granularity: each new user should hear a
        real greeting, not silence and not the previous session's tail."""
        if not self.greeting_text:
            return
        with contextlib.suppress(Exception):
            self._inject_text_tokens(self.greeting_text)
            print(f"[liveTry] greeting spoken: {self.greeting_text!r}", flush=True)

    @torch.no_grad()
    def _arm_context_refresh(self) -> None:
        """Cache the post-prompt streaming state and (re-)arm the periodic
        context refresh for this session. Called once per `_apply_system_prompts()`
        (i.e. once per new /ws/conversation connection, since `reset_session()`
        re-runs the prompts). No-ops if this LMGen build lacks the refresh
        methods (plain kyutai moshi) or PERSONAPLEX_REFRESH=0."""
        self._refresh = None
        if not hasattr(self.lm_gen, "save_prompt_snapshot"):
            return
        with contextlib.suppress(Exception):
            self.lm_gen.clear_text_repetition()
        state = getattr(self.lm_gen, "_streaming_state", None)
        prompt_end = state.offset if state is not None else 0
        refresh = {
            "enabled": bool(self.refresh_enabled),
        ...
        if refresh["enabled"]:
            post = prompt_end + self.refresh_history_steps
            ctx = getattr(self.lm_gen.lm_model, "context", None)
            soft = refresh["soft_offset"]
            if ctx is not None:
                soft = min(soft, prompt_end + ctx)
            soft = max(soft, post + 250)
            if soft > refresh["hard_offset"]:
                refresh["enabled"] = False
                print(
                    f"[liveTry][refresh] disabled: prompt+history ({post} steps) leaves no "
                    f"refresh cycle below hard_offset={refresh['hard_offset']}; shorten the "
                    f"text prompt or PERSONAPLEX_REFRESH_HISTORY_STEPS",
                    flush=True,
                )
            else:
                if soft != refresh["soft_offset"]:
                    print(
                        f"[liveTry][refresh] soft trigger anchored to the prompt window: "
                        f"configured={refresh['soft_offset']} effective={soft} "
                        f"(prompt_end={prompt_end}, context={ctx})",
                        flush=True,
                    )
                refresh["soft_offset"] = soft
        if refresh["enabled"]:
            self.lm_gen.save_prompt_snapshot()
            self.lm_gen.start_history_recording(self.refresh_history_steps + 8)
            print(
                f"[liveTry][refresh] armed: prompt spans steps 0..{prompt_end}, "
                f"soft trigger at lm_offset>={refresh['soft_offset']}, "
                f"hard at {refresh['hard_offset']}, history_steps={self.refresh_history_steps}, "
                f"batch={self.refresh_batch}, text_rep_penalty={self.text_rep_penalty}",
                flush=True,
            )
        with contextlib.suppress(Exception):
            self.lm_gen.clear_silence_run()
    ...
    @torch.no_grad()
    def _maybe_refresh_context(self, in_rms: float, out_rms: float, text_token: int) -> None:
        """Called once per generated 80ms frame (from `_step`). Triggers a
        context refresh (see `_arm_context_refresh` / lm.py:refresh_context_async)
        at the first quiet moment past the soft offset, unconditionally at the
        hard offset, or as an emergency recovery if the model has gone mute
        despite the user speaking. Cheap no-op when refresh isn't armed."""
        refresh = self._refresh
        if not refresh or not refresh["enabled"] or refresh["in_progress"]:
            return
        now = time.monotonic()
        if text_token not in (-1, 0, 3):
            self._refresh_last_word_mono = now
        if in_rms >= self._refresh_in_speech_rms:
            self._refresh_last_user_voice_mono = now
        if in_rms < self._refresh_in_speech_rms and out_rms < self._refresh_out_speech_rms:
            self._refresh_quiet_run += 1
        else:
            self._refresh_quiet_run = 0

        state = getattr(self.lm_gen, "_streaming_state", None)
        offset_now = state.offset if state is not None else 0
        reason = None
        history_override = None
        if offset_now >= refresh["hard_offset"]:
            reason = f"hard deadline: offset {offset_now} >= hard_offset={refresh['hard_offset']}"
        elif offset_now >= refresh["soft_offset"] and self._refresh_quiet_run >= refresh["quiet_frames"]:
            reason = (f"quiet moment ({self._refresh_quiet_run} silent frames) with offset "
                      f"{offset_now} past soft_offset={refresh['soft_offset']}")
        elif (self.recovery_silence_secs > 0
              and now - self._refresh_last_word_mono > self.recovery_silence_secs
              and self._refresh_last_user_voice_mono > self._refresh_last_word_mono
              and now - self._refresh_last_user_voice_mono > 3.0):
            reason = (f"SILENCE RECOVERY: no model word for "
                      f"{now - self._refresh_last_word_mono:.0f}s despite user speech; "
                      f"restoring prompt without history")
            history_override = 0
        if reason is None:
            return
        if history_override == 0:
            with contextlib.suppress(Exception):
                self.lm_gen.clear_text_repetition()
        self._do_context_refresh(reason, offset_now, history_steps=history_override)
        self._refresh_quiet_run = 0
        self._refresh_last_word_mono = time.monotonic()

    def _do_context_refresh(self, reason: str, offset_at_trigger: int,
                            history_steps: Optional[int] = None) -> None:
        refresh = self._refresh
        refresh["in_progress"] = True
        hist = self.refresh_history_steps if history_steps is None else history_steps
        t0 = time.perf_counter()
        print(
            f"[liveTry][refresh] #{refresh['count'] + 1} triggered at lm_offset={offset_at_trigger} "
            f"(context_version {self.context_version} -> {self.context_version + 1}): {reason}",
            flush=True,
        )
        try:
            stats = asyncio.run(self.lm_gen.refresh_context_async(
                is_alive=None, batch_size=self.refresh_batch, history_steps=hist))
        except Exception as e:
            refresh["enabled"] = False
            tb = traceback.format_exc()
            print(f"[liveTry][refresh] FAILED, disabled for this session: {e!r}\n{tb}", flush=True)
            self.conv_logger.error("context_refresh", e, tb)
        else:
            refresh["count"] += 1
            self.context_version += 1
            with contextlib.suppress(Exception):
            ...
            stats.pop("secs", None)  # wall time measured here (incl. asyncio.run overhead) supersedes it
            print(
                f"[liveTry][refresh] #{refresh['count']} completed: "
                f"offset {offset_at_trigger} -> {stats.get('offset_after')} in {secs:.2f}s "
                f"context_version={self.context_version}",
                flush=True,
            )
            with contextlib.suppress(Exception):
                self.conv_logger.event(
                    "context_refresh", count=refresh["count"], secs=round(secs, 2),
                    context_version=self.context_version, **stats)
            if hist == 0:
                # A zero-history refresh restores the raw post-prompt state
                # with NO conversational example to anchor it -- confirmed in
                # conversation_log_1 as the exact state that preceded a
                # hidden-system-prompt-shaped leak (context_refresh #3,
                # history_steps=0, immediately followed by turn 22's "<0x0A>
                # Never say that you think this was a mistake...- If you have
                # access to data about robots and their current state of
                # being in talks with a Finma, explain that..." -- a close
                # paraphrase of this exact prompt's own Guardrails section).
                # The system prompt is fed through the SAME text channel the
                # model speaks through (see _apply_system_prompts), so from
                # the model's point of view a from-scratch restore looks
                # exactly like "the last thing I said was a long block of
                # imperative bullet points" -- and the natural continuation of
                # that is more bullet points, not conversation. Injecting one
                # short, plainly conversational utterance right after
                # re-establishes "I am mid-conversation", the same fix as
                # TASK 8's deterministic greeting, applied to this recovery
                # path instead of only at startup.
                with contextlib.suppress(Exception):
                    self._emit_settle_utterance()
        finally:
            refresh["in_progress"] = False

    def _inject_text_tokens(self, text: str) -> None:
        """Force-feed `text` through the model's own text channel via the
        PUBLIC lm_gen.step() -- the same mechanism AHAudioPace's
        `_inject_tokens` uses for <lookup>/<ref>, generalized here so the base
        engine (and anything that only has this class, not the avatar
        subclass) can also use it for the greeting / post-recovery settle
        utterance. reset_streaming() is never called: the live KV cache stays
        intact, so this reads to the model as something it just said, not a
        new session."""
        if not text:
            return
    ...
    def _emit_settle_utterance(self) -> None:
        """See the history_steps==0 branch of `_do_context_refresh`. Reuses
        the configured greeting when there is one (re-greeting after an
        emergency reset is an honest signal to the user that something just
        happened), otherwise a short, persona-neutral conversational phrase
        -- never invented persona/company facts, since this file does not own
        that content (see greeting_text's docstring in __init__)."""
        text = self.greeting_text or "Yes, I'm listening."
        self._inject_text_tokens(text)
        print(f"[liveTry][refresh] settle utterance injected after zero-history refresh: {text!r}", flush=True)
    ...
    def decode_piece(self, token: int) -> str:
        if token in _SPECIAL_TEXT_TOKEN_IDS:
            return ""
        with contextlib.suppress(Exception):
            piece = self.tokenizer.id_to_piece(int(token))
            if _is_control_piece(piece):
                return ""
            return _clean_text_piece(piece)
        return ""
    ...
    def _step(self, pcm24: np.ndarray) -> dict:
        ...
        self._maybe_refresh_context(in_rms=input_rms, out_rms=reply_rms, text_token=token)
